...
По умолчанию камера участника подменяется пустым видеопотоком и остаётся выключенной.
В окне «Подключение к ВКС» можно включить **«Показывать отсчёт в камере»** — тогда
участники увидят крупный обратный отсчёт, фазу и имя текущего докладчика прямо
в плитке участника «Таймер». Звук и тест звука от камеры не зависят.
Между сигналами синтетический микрофон передаёт тишину.

### Звук при работе модератора на одном компьютере

Если модератор слушает встречу в **основном браузере**, а таймер подключён как отдельный
участник, голоса докладчиков могут дублироваться: из основного браузера и из окна
«Таймер докладов — ВКС». В настройках звука есть два независимых переключателя:

- **Не воспроизводить сигналы таймера на этом компьютере** — отключает локальные
  сигналы таймера (chime/bell) через колонки ПК; в ВКС они по-прежнему передаются.
- **Не воспроизводить звук участников в окне ВКС таймера** — отключает голоса
  докладчиков в окне браузера таймера (рекомендуется включить при работе на одном ПК
  с основным браузером).

По умолчанию второй переключатель включён.
...
## Кастомные шрифты цифр таймера

Чтобы добавить новый шрифт перед сборкой:

1. Положите файл шрифта (`.otf`, `.ttf`, `.woff`, `.woff2`) в `frontend/src/assets/fonts/`.
2. Добавьте запись в `frontend/src/assets/fonts/manifest.json`:
...
3. Соберите приложение (`npm run build` в `frontend/`, затем `wails build`).

Vite упакует файл в сборку, а в настройках «Шрифт цифр» появится новый пункт. Идентификатор `id` должен содержать только латинские буквы, цифры, `_` и `-`.
