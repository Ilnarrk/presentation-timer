...
| WebView2 Runtime | обычно уже есть в Windows 11 | Окно приложения и встроенный участник ВКС |
...
Таймер открывает встречу во **встроенном окне WebView2** (тот же движок, что и у самого приложения), входит как участник
`Таймер` и передаёт сигнал через синтетический WebRTC-микрофон. VB-CABLE,
VoiceMeeter и установленный Google Chrome / Яндекс Браузер для этого не нужны.
...
2. Нажмите «Подключиться». Откроется отдельное окно «Presentation Timer — ВКС».
3. Если встреча защищена SSO, CAPTCHA или комнатой ожидания, завершите вход вручную в этом окне.
...
Открыть диалог повторно можно нажатием на статус ВКС над таймером. Красная
кнопка в окне отключает участника от встречи. Закрытие окна ВКС тоже завершает сессию.
...
### Для центра информационной безопасности

При подключении к ВКС приложение:

- открывает **собственное окно WebView2** с изолированным профилем в `%AppData%\presentation-timer\conference-webview\`;
- использует Chrome DevTools Protocol **только на 127.0.0.1** (случайный локальный порт) для передачи тестового звука в WebRTC;
- **не** запускает PowerShell, **не** перечисляет процессы через WMI/CIM и **не** запускает внешний Chrome/Edge пользователя.

Ожидаемые процессы: `presentation-timer.exe` и штатные подпроцессы WebView2 (`msedgewebview2.exe`), как у любого приложения на Wails/WebView2.
...
- Профиль встроенного окна хранится в `%AppData%\presentation-timer\conference-webview`.
- Для SaluteJazz окно представляется как Google Chrome: платформа не принимает Edge/WebView2.
- Firefox не используется: синтетический микрофон таймера работает через CDP в WebView2.
- Таймер подтверждает отправку данных в окно ВКС, но платформы не возвращают надёжную
...
- На внутренних серверах с самоподписанным сертификатом WebView2 может отказать
  открыть страницу — это ограничение браузерного движка, не приложения.
...
| Не открывается участник ВКС | Установите [Microsoft Edge WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/) |
